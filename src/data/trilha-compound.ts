...
export const compoundTopics: Topic[] = [
  {
    ...
    priority: 'alta',
    type: 'conceito',
    shortDescription: 'O princípio que inverte a dívida técnica — features ensinam o sistema em vez de complicá-lo.',
    concept:
    ...
    projectContext: 'É a tese que sustenta a promessa do Adapta Native: entregar ao cliente um sistema que melhora sozinho a cada iteração, em vez de um monólito que apodrece.',
    references: [
      { label: 'Comece aqui: o guia definitivo de Compound Engineering (Every)', url: 'https://every.to/guides/compound-engineering', kind: 'artigo' },
      { label: 'Depois leia: como a Every coda com agentes (a filosofia aplicada)', url: 'https://every.to/chain-of-thought/compound-engineering-how-every-codes-with-agents', kind: 'artigo' },
    ],
    xp: 60,
    estMinutes: 30,
  },
  {
    ...
    priority: 'alta',
    type: 'conceito',
    shortDescription: 'As crenças-base: trabalho que compõe, gosto em sistemas, ensinar em vez de fazer, redes de segurança, ambiente agent-native.',
    concept:
    ...
    projectContext: 'Os cinco princípios viram o checklist de maturidade que a consultoria aplica no ambiente do cliente antes de escalar o uso de agentes.',
    references: [
      { label: 'Comece aqui: os princípios que sustentam a abordagem', url: 'https://every.to/guides/compound-engineering', kind: 'artigo' },
      { label: 'Aprofunde: gosto em sistemas, não em review (a prática)', url: 'https://every.to/source-code/my-ai-had-already-fixed-the-code-before-i-saw-it', kind: 'artigo' },
    ],
    xp: 60,
    estMinutes: 30,
  },
  {
    ...
    priority: 'alta',
    type: 'conceito',
    shortDescription: 'Onde você está: de escrever linha a linha até delegar tarefas autônomas com revisão multi-agente.',
    concept:
    ...
    projectContext: 'A escada é a ferramenta de diagnóstico de maturidade que a consultoria usa no discovery — dá pra vender a jornada de adoção degrau a degrau, não um salto impossível.',
    references: [
      { label: 'Comece aqui: os 5 estágios da escada de adoção', url: 'https://every.to/guides/compound-engineering', kind: 'artigo' },
      { label: 'Veja aplicado: os fluxos de IA dos 6 engenheiros da Every', url: 'https://every.to/source-code/inside-the-ai-workflows-of-every-s-six-engineers', kind: 'artigo' },
    ],
    xp: 60,
    estMinutes: 30,
  },
  {
    ...
    projectContext: 'O loop é o método de trabalho que a consultoria instala no time do cliente — plan-work-review-compound vira o processo padrão de toda entrega com agente.',
    references: [
      { label: 'Comece aqui: o loop plan-work-review-compound explicado', url: 'https://every.to/guides/compound-engineering', kind: 'artigo' },
      { label: 'A evolução do loop (de 4 para 8 passos, o "sanduíche")', url: 'https://every.to/p/compound-engineering-gets-an-upgrade', kind: 'artigo' },
    ],
    xp: 60,
    estMinutes: 35,
  },
  {
    ...
    priority: 'alta',
    type: 'pratica',
    shortDescription: 'Planos como documentos revisáveis, planejamento multi-agente e o plan mode do Claude Code.',
    concept:
    ...
    projectContext: 'O "plano revisável antes de codar" é entregável e ritual da consultoria — no Adapta Native, o plano aprovado é a ponte entre a spec do cliente e o código.',
    references: [
      { label: 'Comece aqui: plan mode do Claude Code (planejar antes de agir)', url: CC + '/common-workflows', kind: 'doc' },
      { label: 'Aprofunde: planejamento multi-agente no fluxo da Every', url: 'https://every.to/chain-of-thought/compound-engineering-how-every-codes-with-agents', kind: 'artigo' },
      { label: 'Subagentes para pesquisa paralela (Claude Code)', url: CC + '/sub-agents', kind: 'doc' },
    ],
    xp: 60,
    estMinutes: 35,
  },
  {
    ...
    priority: 'alta',
    type: 'pratica',
    shortDescription: 'Deixar o agente executar o plano aprovado, e calibrar quando ele pede permissão.',
    concept:
    ...
    projectContext: 'A matriz de autonomia calibrada é parte da governança que a consultoria entrega ao cliente — produtividade com trilho, não anarquia de agente.',
    references: [
      { label: 'Comece aqui: permissões e modos de permissão (Claude Code)', url: CC + '/permissions', kind: 'doc' },
      { label: 'Aprofunde: a pasta é o agente (44 agentes na prática)', url: 'https://every.to/source-code/the-folder-is-the-agent', kind: 'artigo' },
  ...
  },
  {
    ...
    priority: 'alta',
    type: 'pratica',
    shortDescription: 'Review multi-agente pega o técnico; o humano foca no que combinamos construir.',
    concept:
    ...
    projectContext: 'O sistema de review multi-agente é o que garante a qualidade prometida ao cliente na era do código gerado — e é entregável interno da consultoria.',
    references: [
      { label: 'Comece aqui: review focado em intenção (guia)', url: 'https://every.to/guides/compound-engineering', kind: 'artigo' },
      { label: 'Aprofunde: 13 agentes em paralelo pegaram um bug crítico', url: 'https://every.to/source-code/my-ai-had-already-fixed-the-code-before-i-saw-it', kind: 'artigo' },
      { label: 'Como criar subagentes revisores (Claude Code)', url: CC + '/sub-agents', kind: 'doc' },
    ],
    xp: 60,
    estMinutes: 35,
  },
  {
    ...
    priority: 'alta',
    type: 'pratica',
    shortDescription: 'A fase que dá nome ao método — cada feedback vira CLAUDE.md, check ou subagente.',
    concept:
    ...
    projectContext: 'O "sistema que aprende" é o diferencial vendável do Adapta Native: a cada entrega, o repositório do cliente fica mais inteligente — a fase compound é onde essa mágica acontece.',
    references: [
      { label: 'Comece aqui: a IA que já tinha corrigido antes de eu ver', url: 'https://every.to/source-code/my-ai-had-already-fixed-the-code-before-i-saw-it', kind: 'artigo' },
      { label: 'Onde codificar: memória do Claude Code (CLAUDE.md)', url: CC + '/memory', kind: 'doc' },
      { label: 'Hooks para travar regras de forma determinística', url: CC + '/hooks', kind: 'doc' },
    ],
    xp: 60,
    estMinutes: 35,
  },
  {
    ...
    priority: 'media',
    type: 'pratica',
    shortDescription: 'CLAUDE.md + skills + contexto acumulado transformam um modelo genérico em especialista.',
    concept:
    ...
    projectContext: 'O modelo "cliente = pasta-agente que acumula" é como a consultoria escala: cada projeto deixa a pasta do cliente mais capaz, e o próximo sai mais rápido.',
    references: [
      { label: 'Comece aqui: a pasta é o agente (Every)', url: 'https://every.to/source-code/the-folder-is-the-agent', kind: 'artigo' },
      { label: 'Skills no Claude Code (capacidades reutilizáveis)', url: CC + '/skills', kind: 'doc' },
  ...
  },
  {
    ...
    priority: 'media',
    type: 'pratica',
    shortDescription: 'Planos e reviews sem reunião, e passagens de bastão que carregam todo o contexto.',
    concept:
    ...
    projectContext: 'Handoff perfeito é o que permite entregar e transferir projetos ao cliente sem depender de você de plantão — é a base do handover limpo da trilha de Projetos.',
    references: [
      { label: 'Comece aqui: trabalho assíncrono e handoffs (guia)', url: 'https://every.to/guides/compound-engineering', kind: 'artigo' },
      { label: 'Retomar sessões e estado em arquivos (Claude Code)', url: CC + '/common-workflows', kind: 'doc' },
    ],
    xp: 45,
    estMinutes: 25,
  },
  {
    ...
    priority: 'media',
    type: 'pratica',
    shortDescription: 'Subagentes e skills prontos que operacionalizam o loop — e como adaptar ao contexto de vocês.',
    concept:
    ...
    projectContext: 'O plugin é o molde a partir do qual a consultoria monta seu próprio kit de subagentes e comandos — o "template de repo profissional" da trilha de Projetos ganha o método compound embutido.',
    references: [
      { label: 'Comece aqui: o plugin open-source de compound engineering', url: 'https://github.com/EveryInc/compound-engineering-plugin', kind: 'tool' },
      { label: 'Referência do método por trás do plugin (guia)', url: 'https://every.to/guides/compound-engineering', kind: 'artigo' },
      { label: 'Como criar seus próprios subagentes (Claude Code)', url: CC + '/sub-agents', kind: 'doc' },
    ],
    xp: 45,
    estMinutes: 35,
  },
  {
    ...
    priority: 'alta',
    type: 'missao',
    shortDescription: 'Missão: instalar o loop plan-work-review-compound num projeto real de cliente.',
    concept:
    ...
    projectContext: 'É a prática que transforma o método em capacidade real da consultoria — e o resultado (um repo que melhora sozinho) é exatamente o que o Adapta Native promete ao cliente.',
    references: [
      { label: 'Referência do loop completo (guia definitivo)', url: 'https://every.to/guides/compound-engineering', kind: 'artigo' },
      { label: 'Best practices do Claude Code (para as redes de segurança)', url: CC + '/best-practices', kind: 'doc' },
    ],
    xp: 60,
    estMinutes: 240,
  },
  {
    ...
    priority: 'alta',
    type: 'boss',
    shortDescription: 'Desafio final — a filosofia, os 5 princípios, a escada, o loop e a fase compound.',
    concept:
    ...
    pitfalls: [],
    practiceSteps: [],
    projectContext: 'Aprovação indica que o consultor internalizou o compound engineering e consegue instalá-lo no time e no repositório do cliente.',
    references: [],
    xp: 120,
    estMinutes: 15,
  },
